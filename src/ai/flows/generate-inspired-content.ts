...
const GenerateInspiredContentOutputSchema = z.discriminatedUnion("analysisMode", [
  z.object({ analysisMode: z.literal('brainstormIdeas'), ...BrainstormOutputSchema.shape }),
  z.object({ analysisMode: z.literal('synthesizeOutline'), ...SynthesizeOutputSchema.shape })
]);
...
const prompt = ai.definePrompt({
  ...
  output: {schema: GenerateInspiredContentOutputSchema},
...
Based on the user's selected 'analysisMode', perform ONE of the following tasks:
...
IF 'analysisMode' is 'brainstormIdeas':
  ...
  - Structure your output according to the 'BrainstormOutputSchema'.
...
IF 'analysisMode' is 'synthesizeOutline':
  ...
  - Structure your output according to the 'SynthesizeOutputSchema'.

Ensure your output strictly matches the required Zod schema for the chosen 'analysisMode'.
`,
});
...
const generateInspiredContentFlow = ai.defineFlow(
  {
    ...
    outputSchema: GenerateInspiredContentOutputSchema,
  },
  async (input: GenerateInspiredContentInput) => {
    // In a real scenario with actual scraping, pre-processing of URL content would happen here.
    // For now, the prompt simulates that the AI has access to this content.
    const {output} = await prompt(input);

    if (!output) {
        throw new Error("AI failed to generate inspired content.");
    }
    // Ensure the output matches the discriminated union structure
    if (input.analysisMode === 'brainstormIdeas') {
        return { analysisMode: 'brainstormIdeas', ...output } as GenerateInspiredContentOutput;
    } else {
        return { analysisMode: 'synthesizeOutline', ...output } as GenerateInspiredContentOutput;
    }
  }
);
