...
import { getAllProductCategories, addProductCategory } from '@/lib/product-categories-data';
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error("API - Failed to fetch product categories:", error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const addedCategory = await addProductCategory(newCategoryData);
    return NextResponse.json(addedCategory, { status: 201 });
  } catch (error: any) {
    console.error("API - Failed to create product category:", error);
