...
import { CheckCircle, XCircle, Loader2, CalendarClock, User, Mail, Info, Edit, Briefcase } from "lucide-react";
...
export default function MyBookingsPage() {
  ...
  const fetchBookings = useCallback(async () => {
    ...
    try {
      ...
      toast({ title: "Error", description: error.message || "Could not load your bookings.", variant: "destructive" });
      setBookings([]);
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="container mx-auto py-8">
      <header className="mb-8">
        <h1 className="text-4xl font-bold tracking-tight font-headline flex items-center">
          <CalendarClock className="mr-3 h-10 w-10 text-primary" />
          Manage My Bookings
        </h1>
      ...
      <Card className="shadow-xl mb-8">
        ...
        <CardContent>
          {bookings.length === 0 ? (
            <p className="text-muted-foreground text-center py-4">No booking requests found for your services.</p>
          ) : (
