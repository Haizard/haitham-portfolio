...
import { Loader2, Upload, Plus, X, Star, HelpCircle, Map, Lightbulb, ArrowLeft, Check } from 'lucide-react';
    ...
    useEffect(() => {
        const fetchTour = async () => {
            try {
                ...
                toast({
                    title: "Error",
                    description: error.message || "Failed to load tour details",
                    variant: "destructive",
                });
    ...
    return (
        <div className="container max-w-5xl mx-auto py-8 px-4">
            <div className="mb-8 flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-3xl font-bold mb-2">Edit Tour Package</h1>
                    <p className="text-muted-foreground">Modify your tour itinerary and availability.</p>
                </div>
            </div>
                                ...
                                <FormField control={form.control} name="isActive" render={({ field }) => (
                                    <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4 mt-auto">
                                        ...
                                        <div className="space-y-1 leading-none">
                                            <FormLabel>Tour Active</FormLabel>
                                            <FormDescription>If disabled, this tour won't appear in search results.</FormDescription>
                                        </div>
                                    </FormItem>
                                )} />
                        ...
                        <Card>
                            ...
                            <CardContent className="space-y-4">
                                <div className="flex gap-2">
                                    ...
                                    <Button type="button" onClick={addHighlight} size="sm" variant="outline"><Plus className="w-4 h-4" /></Button>
                                </div>
                                <ul className="list-disc pl-5 space-y-1">
                                    {highlights.map((h, i) => (
                                        <li key={i} className="text-sm flex justify-between">
                                            {h.text}
                                            <X className="w-4 h-4 cursor-pointer text-red-500 hover:scale-110" onClick={() => setHighlights(highlights.filter((_, idx) => idx !== i))} />
                                        </li>
                                    ))}
                                </ul>
                            </CardContent>
                        ...
                        <Card>
                            ...
                            <CardContent className="space-y-4">
                                ...
                                <div className="space-y-2 max-h-60 overflow-y-auto">
                                    {faqs.map((f, i) => (
                                        <div key={i} className="bg-muted p-2 rounded relative group border">
                                            <p className="font-semibold text-xs">{f.question}</p>
                                            <p className="text-xs text-muted-foreground">{f.answer}</p>
                                            <X className="w-4 h-4 absolute top-1 right-1 cursor-pointer opacity-0 group-hover:opacity-100 text-red-500" onClick={() => setFaqs(faqs.filter((_, idx) => idx !== i))} />
                                        </div>
                                    ))}
                                </div>
                            </CardContent>
                            ...
                            <CardContent className="grid md:grid-cols-2 gap-6">
                                <div className="space-y-4">
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        <Button type="button" onClick={() => addItem(itineraryInput, setItineraryInput, itinerary, setItinerary)} className="h-20" variant="outline">Add</Button>
                                    </div>
                                    <div className="space-y-2">
                                        {itinerary.map((item, i) => (
                                            <div key={i} className="bg-muted p-2 rounded text-sm relative group border">
                                                <span className="font-bold mr-2 text-primary">Day {i + 1}:</span> {item}
                                                <X className="w-4 h-4 absolute top-1 right-1 cursor-pointer opacity-0 group-hover:opacity-100 text-red-500" onClick={() => setItinerary(itinerary.filter((_, idx) => idx !== i))} />
                                            </div>
                                        ))}
                                    </div>
                                </div>
                                ...
                                <div className="space-y-6">
                                    <div className="space-y-2">
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {inclusions.map((item, i) => (
                                                <span key={i} className="bg-green-100 text-green-800 text-xs px-2 py-1 rounded flex items-center gap-1 border border-green-200">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {exclusions.map((item, i) => (
                                                <span key={i} className="bg-red-100 text-red-800 text-xs px-2 py-1 rounded flex items-center gap-1 border border-red-200">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {tags.map((item, i) => (
                                                <span key={i} className="bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded flex items-center gap-1 border border-blue-200">
                            ...
                            <CardContent className="space-y-6">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        <Button type="button" onClick={addGalleryItem} variant="outline"><Upload className="w-4 h-4 mr-2" /> Add</Button>
                                    </div>
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-2">
                                        {gallery.map((img, i) => (
                                            <div key={i} className="relative group border rounded p-2 bg-muted/20">
                                                ...
                                                <button
                                                    type="button"
                                                    className="w-5 h-5 absolute -top-2 -right-2 bg-red-500 text-white rounded-full flex items-center justify-center cursor-pointer shadow-sm border border-white"
