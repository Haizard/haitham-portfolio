...
import { PlusCircle, Edit3, Trash2, Loader2, ChevronDown, ChevronRight, FolderPlus, ListTree, Package } from "lucide-react";
...
export function ProductCategoryManagement() {
  ...
  const [editingNode, setEditingNode] = useState<ProductCategoryNode | null>(null);
  const [parentForNewNode, setParentForNewNode] = useState<{ id: string | null; name: string | null } | null>(null);
  
  const [nodeToDelete, setNodeToDelete] = useState<ProductCategoryNode | null>(null);
  ...
  const fetchCategoryTree = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error('Failed to fetch product category tree');
      const tree: ProductCategoryNode[] = await response.json();
      setCategoryTree(tree);
    } catch (error) {
      console.error(error);
      toast({ title: "Error", description: "Could not load product category tree.", variant: "destructive" });
      setCategoryTree([]);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleCreateChildCategory = (parentNode: ProductCategoryNode) => {
    setEditingNode(null);
    setParentForNewNode({ id: parentNode.id!, name: parentNode.name });
    setIsFormOpen(true);
  };

  const handleEditNode = (node: ProductCategoryNode) => {
    setEditingNode(node);
    setParentForNewNode(null); 
    setIsFormOpen(true);
  };

  const confirmDeleteNode = (node: ProductCategoryNode) => {
  ...
  const handleDeleteNode = async () => {
    ...
    setIsDeleting(true);
    try {
      const response = await fetch(`/api/product-categories/${nodeToDelete.id}`, { method: 'DELETE' });
      if (!response.ok) {
  ...
  const renderCategoryRowsRecursive = (nodes: ProductCategoryNode[], level = 0): React.ReactNode[] => {
    ...
    <TooltipProvider>
      <Card className="shadow-xl mb-8">
        <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <CardTitle className="text-2xl font-headline flex items-center"><ListTree className="mr-2 h-6 w-6 text-primary"/>Product Category Tree</CardTitle>
            <CardDescription>Manage your hierarchical store categories.</CardDescription>
          </div>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="min-w-[100px] text-center">Product Count</TableHead>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              and all of its descendant categories/subcategories. Any products in these categories will need to be re-categorized manually.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
