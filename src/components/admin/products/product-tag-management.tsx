...
import { PlusCircle, Edit3, Trash2, Loader2, Tags as TagsIcon, Package } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import type { ProductTag } from '@/lib/product-tags-data';
import { ProductTagFormDialog } from './product-tag-form-dialog';
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export function ProductTagManagement() {
  ...
  const fetchTags = useCallback(async () => {
    ...
    try {
      ...
      console.error(error);
      toast({ title: "Error", description: "Could not load product tags.", variant: "destructive" });
      setTags([]);
    } finally {
      setIsLoading(false);
    }
  ...
  const confirmDeleteTag = (tag: ProductTag) => {
    setTagToDelete(tag);
  };
  ...
  const handleDeleteTag = async () => {
    ...
    setIsDeleting(true);
    try {
      const response = await fetch(`/api/product-tags/${tagToDelete.id}`, { method: 'DELETE' });
      if (!response.ok) {
    ...
    <TooltipProvider>
      <Card className="shadow-xl mb-8">
        <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <CardTitle className="text-2xl font-headline flex items-center"><TagsIcon className="mr-2 h-6 w-6 text-primary"/>Product Tag List</CardTitle>
            <CardDescription>Manage all product tags.</CardDescription>
          </div>
          <Button onClick={handleCreateTag} className="bg-primary hover:bg-primary/90 w-full sm:w-auto">
            <PlusCircle className="mr-2 h-5 w-5" /> Create New Product Tag
          </Button>
        </CardHeader>
        <CardContent>
          {tags.length === 0 ? (
            <p className="text-muted-foreground text-center py-4">No product tags found. Get started by creating one!</p>
          ) : (
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              Products using this tag will have the tag unlinked.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
