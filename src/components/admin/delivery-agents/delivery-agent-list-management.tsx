...
export function DeliveryAgentListManagement() {
  ...
  const fetchAgents = useCallback(async () => {
    setIsLoading(true);
    try {
      const response = await fetch('/api/users/by-role/delivery_agent');
      if (!response.ok) throw new Error('Failed to fetch delivery agents');
      const data: FreelancerProfile[] = await response.json();
      setAgents(data);
    } catch (error: any) {
      console.error("Error fetching delivery agents:", error);
      toast({ title: "Error", description: error.message || "Could not load delivery agents.", variant: "destructive" });
      setAgents([]);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleUpdateStatus = async (agentId: string, newStatus: VendorStatus) => {
  ...
  return (
    <Card className="shadow-xl">
      <CardHeader>
        <CardTitle>Delivery Agent Applications & Status</CardTitle>
        <CardDescription>Review new applications and manage existing delivery agents.</CardDescription>
      </CardHeader>
      <CardContent>
        {agents.length === 0 ? (
          <p className="text-muted-foreground text-center py-4">No delivery agents have registered yet.</p>
        ) : (
