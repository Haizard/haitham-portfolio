...
interface ServiceCategoryFormDialogProps {
  isOpen: boolean;
  onClose: () => void;
  categoryNode?: ServiceCategoryNode | null; 
  parentId?: string | null;          
  parentName?: string | null;        
  onSuccess: () => void;
}

export function ServiceCategoryFormDialog({ isOpen, onClose, categoryNode, parentId, parentName, onSuccess }: ServiceCategoryFormDialogProps) {
  ...
  const handleSubmit = async (values: CategoryFormValues) => {
    ...
    const apiUrl = isEditing && categoryNode?.id ? `/api/service-categories/${categoryNode.id}` : '/api/service-categories';
  ...
  const getDialogTitle = () => {
    if (categoryNode) return "Edit Service Category";
    if (parentName) return `Create New Category under "${parentName}"`;
    return "Create New Top-Level Service Category";
  };
  
  const getDialogDescription = () => {
    if (categoryNode) return "Update the details of this service category.";
    return "Fill in the details for your new service category.";
  };
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <Input id="name" placeholder="e.g., Web Development Services" {...field} />
                  <FormMessage />
                </FormItem>
              )}
            />
